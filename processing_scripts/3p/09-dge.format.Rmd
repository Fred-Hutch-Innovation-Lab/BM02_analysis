...
1. CD4+ T vs. CD8+ T 
2. B naive vs. B memory 
3. B memory vs. CD4+ T 
4. B vs. T

I show the overlap significantly expressed genes and a few marker genes. I don't think more DGE is necessarily better or worse, as the validity of the results matter, hence why looking at 'ground truth' may be more valuable. However, these 'cannonical' genes were also used to call these celltypes during celltype annotation, so it becomes a bit circular. So I figured more high-level comparisons (e.g. B v T) may be better than the lower level comparisons. Even if detecting that signal is valuable, reporting on that in the pseudobulked data may be a bit redundant and subjective to the cell-calling difficulties. However, every kit SHOULD (and did) have good separation of T and B cells, so that *should* be a valid comparison. We could maybe do T v B, B v monocyte, and T v monocyte as those 3 tend to be easy to distinguish. 
