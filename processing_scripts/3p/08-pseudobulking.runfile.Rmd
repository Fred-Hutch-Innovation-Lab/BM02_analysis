...
```{r}
corr_scatterplot <- function() {
  plotdata <- assays(pb_obj)$vst |> 
    assay() |> 
    melt() |> 
    dplyr::rename(Gene = Var1) |>
    separate(Var2, into = c('Sample', 'Celltype'), sep='_') |>
    mutate(Sample = gsub('-', '_', Sample, fixed=TRUE)) |>
    merge(metadata, by='Sample')
  plotdata <- merge(plotdata, plotdata, by = c('Kit', 'Gene'))
...
## Invidiual variance genes

```{r}
features <- results(pb_obj, name = 'Individual_F5_vs_F1') |> as.data.frame() |> arrange(padj) |> head(n=500) |> rownames()
for (kit in unique(metadata$Kit)) {
  plotdata <- tibble()
...
features <- unique(unname(unlist(pathways)))
for (kit in unique(metadata$Kit)) {
  plotdata <- tibble()
  for (celltype in unique(colData(objs[[kit]])$celltype)) {
    if (nrow(filter(cellcounts_coarse_kit, Kit == kit, Celltype == celltype)) > 0){
      ...
      #   head(n=200) |> 
      #   rownames()
      plotdata1 <- obj_cor(assays(objs[[kit]])$vst, kit, celltype, ngenes=2000, method='pearson', features=features, features_select = 'kit+celltype') |>
      filter(celltype1 == celltype2) |>
      # filter((Individual1 == 'F1A' & Individual2 == 'F1B') |
      #      (Individual1 == 'F5A' & Individual2 == 'F5B')) |>
      mutate(celltype1 = factor(celltype1, levels=label_order),
