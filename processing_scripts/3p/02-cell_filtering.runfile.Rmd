...
```{r}
objs <- readRDS(here('rds/3p/01_raw_objs.rds'))
...
names(objs) <-
  names(objs) %>%
    case_when(
      grepl('FBv4', .) ~ gsub('FBv4', 'Fluent_v4', .),
      grepl('FBv5', .) ~ gsub('FBv5', 'Fluent_V', .),
      grepl('PA_V3', .) ~ gsub('PA_V3', 'Parse_v3', .),
      .default = .
    )
names(objs) <-
  names(objs) %>%
...
objs <- objs[names(objs)[!grepl('BD', names(objs))]]
names(objs)
...
# Metric distribution comparisons

```{r}
plotdata <- lapply(objs, function(x) {
  x@meta.data |> 
  as.data.frame() |> 
  select(orig.ident, nCount_RNA, nFeature_RNA, log10GenesPerUMI, mtRatio, rbRatio)
}) |> data.table::rbindlist() |>
  merge(metadata, by.x='orig.ident', by.y='Sample')
```
...
```{r}
# tmp <- plotdata |>
#   group_by(Kit, Individual, Replicate) |>
#   select(-Sensitivity) 

summary(lm(nCount_RNA ~ Kit + Individual + Replicate, data = plotdata))
summary(lm(nFeature_RNA ~ Kit + Individual + Replicate, data = plotdata))
summary(lm(log10GenesPerUMI ~ Kit + Individual + Replicate, data = plotdata))
summary(lm(mtRatio ~ Kit + Individual + Replicate, data = plotdata))
summary(lm(rbRatio ~ Kit + Individual + Replicate, data = plotdata))

  # summarize(across(where(is.numeric), median, na.rm = TRUE), .groups = "drop") 
  #|>
  # group_by(Kit) |>
  # summarize(across(where(is.numeric), list(mean = mean, sd = sd), na.rm = TRUE), .groups = "drop") 
```
...
```{r}
ks_res <- kruskal.test(nCount_RNA ~ Kit, plotdata)
d_res1 <- dunn.test::dunn.test(plotdata$nCount_RNA,plotdata$Kit, method = 'bonferroni')
d_res2 <- dunn.test::dunn.test(plotdata$nFeature_RNA,plotdata$Kit, method = 'bonferroni')
d_res3 <- dunn.test::dunn.test(plotdata$log10GenesPerUMI,plotdata$Kit, method = 'bonferroni')
d_res4 <- dunn.test::dunn.test(plotdata$mtRatio,plotdata$Kit, method = 'bonferroni')
d_res5 <- dunn.test::dunn.test(plotdata$rbRatio,plotdata$Kit, method = 'bonferroni')
```
...
```{r}
saveRDS(objs_filt, here('rds/3p/02-objs_post_cell_filtering.rds'), compress=FALSE)
```
...
## aesthetics 
rmarkdown::render(here('processing_scripts/3p/02-cell_filtering.format.Rmd'),
