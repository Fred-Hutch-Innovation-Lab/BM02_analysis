...
```{r}
cellcounts <- lapply(objs, function(x) {
  table(x$orig.ident, x$cell_labels.fine) |>
    as.data.table() 
}) |> rbindlist(idcol = 'Kit')
colnames(cellcounts) <- c('Kit', 'Sample', 'Celltype', 'Count')
cellcounts_kit <- cellcounts |>
  group_by(Kit) |> 
    mutate(prop = Count / sum(Count)) |>
  group_by(Kit, Celltype) |> 
  summarize(prop = sum(prop))
cellcounts_samp <- cellcounts |>
  group_by(Sample) |> 
    mutate(prop = Count / sum(Count)) |>
  group_by(Sample, Celltype) |> 
  summarize(prop = sum(prop))
write.table(cellcounts, 
...
coldata <- obj@Dimnames[[2]] |> 
  ...
  mutate(celltype = factor(celltype, levels = coarse_celltypes$fine))

design_formula <- as.formula('~ Kit + celltype + Individual * Replicate')
...
plotdata <- pca(assay(vsd), npcs=20, nfeatures = 200) |>
  as.data.frame() |>
...
```{r}
  plotdata <- 
    pca(assay(vsd), nfeatures = 200, npcs = 20) |>
    dist() |>
    as.matrix() |>
    as.data.frame() |>
  ...
  arrange(celltype1) |>
  filter(!celltype1 %in% c('T', 'B', 'Monocyte', 'Unknown')) |>
...
ggplot(plotdata ,
    ...
    geom_tile() +
    geom_text() +
    scale_fill_gradient2(low = "#4DAF4A",
                       mid="#FFFFCC",
                       high = "#FF0000", 
                       midpoint=max(plotdata$value)/2, limits = c(0, max(plotdata$value))) +
    labs(x = 'Sample', y = 'Celltype', fill = 'Distance') +
    facet_wrap(~ kit1, ncol=7, nrow = 1, labeller = labeller(Kit = label_function)) ->
...
```{r}
euc_data <- expand.grid(unique(metadata$Kit), unique(metadata$Individual), coarse_celltypes$fine) |>
...
for (kit in unique(metadata$Kit)) {
  for (celltype in unique(colData(obj)$celltype)) {
    if (table(obj$Kit, obj$celltype)[kit, celltype] > 0) {
      plotdata <- obj_cor(vsd, kit, celltype, ngenes=200,
                          method='spearman',
...
for (kit in unique(metadata$Kit)) {
  ...
  for (celltype in unique(colData(obj)$celltype)) {
    if (table(obj$Kit, obj$celltype)[kit, celltype] > 0) {
      ...
      #   head(n=200) |> 
      #   rownames()
      plotdata1 <- obj_cor(vsd, kit, celltype, ngenes=2000, method='spearman', features_select = 'kit+celltype') |>
      filter(celltype1 == celltype2) |>
      # filter((Individual1 == 'F1A' & Individual2 == 'F1B') |
      #      (Individual1 == 'F5A' & Individual2 == 'F5B')) |>
      mutate(celltype1 = factor(celltype1, levels=coarse_celltypes$fine),
...
sampleDists <- dist(t(assay(vsd))) |>
  ...
  separate_wider_delim(rn, '_', too_many = 'merge',
    ...
    mutate(Kit1 = gsub('-', '_', Kit1),
           Kit2 = gsub('-', '_', Kit2)) 
  # mutate(Kit1 = factor(Kit1, levels = kit_order_3p),
         # Celltype = factor(celltype1, levels = label_order)) |>
  # select(-c(celltype1, , celltype2)) |>
  # filter(!is.na(celltype))  |>
  # group_by(Kit, Celltype) |>
  # summarize(y=mean(value))  #|>
      # filter((Individual1 == 'F1A' & Individual2 == 'F1B') |
...
# ggplot(aes(x=Individual1, y=Individual2, fill=value)) +
#   geom_tile() +
#   facet_grid(celltype1 ~ Kit1)
