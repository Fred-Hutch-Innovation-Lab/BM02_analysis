...
```{r}
metadata <- metadata %>%
  # filter(!(Sample %in% c('GEM3P_F1'))) %>% ## Data wasn't processed correctly
  filter(!(Sample %in% c('NextGEM5P_F4', 'NextGEM5P_F5'))) 
```

```{r}
stopifnot(all(metadata$Sample %in% names(objs)))
# stopifnot(all(names(objs) %in% metadata$Sample))
objs <- objs[metadata$Sample]
...
# tmp <- tmp[c('Flex','Fluent','GEMX3P','NextGEM3P','Parse_V3')]
...
```{r}
saveRDS(objs_filt, here('rds/02-filtered_objs.rds'), compress=FALSE)
```
