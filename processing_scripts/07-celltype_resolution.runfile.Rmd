...
# objs <- objs[names(objs)[!grepl('BD', names(objs))]]z
...
```{r, fig.width=10}
features <- celltype_markers_modules$`CD4+ T`
...
objs <- lapply(objs, function(x){
  ...
  AddModuleScore_UCell(x,
               features=markers$B, name= '_filtered_UCell',
               storeRanks=TRUE, ncores=4)
})
objs <- lapply(objs, function(x){
  markers <- filter_marker_genes(celltype_markers_modules, Features(x))
  AddModuleScore_UCell(x,
               features=markers, name= '_filtered_UCell', ncores=4)
})
```
