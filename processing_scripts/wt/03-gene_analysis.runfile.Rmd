...
aggregate_gene_expression <- function(obj, layer='data', genes = Features(obj, layer=layer)) {
  ...
  exp <- GetAssayData(obj, assay ='RNA', layer = layer)
  # rownames(exp) <- genes
...
gene_exp_summary1 <- lapply(gene_exp_summary, function(x) {
  ...
  merge(x, info, by.x='row.names', by.y='external_gene_name', all.x=TRUE) |>
...
objs <- lapply(objs, function(x){
  x <- subset(x, features = detected_genes_kit_level[[metadata$Kit[grepl(x@project.name, metadata$Sample)]]])
  x
})
