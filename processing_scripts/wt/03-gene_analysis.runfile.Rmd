...
```{r}
objs <- readRDS(here('rds/wt/02-objs_post_cell_filtering.rds'))
kit_order <- unname(unlist(read.table(here('config/wt/kit_order.txt'))))
...
c("## Set of genes present in all 3' kits (not flex as it is probe based) in at least 10 cells in at least 2 samples",
                  shared_genes) |>
  write.table(here('rds/wt/gene_whitelist.txt'), quote = FALSE, row.names = FALSE, col.names = FALSE)
length(shared_genes)
```
...
```{r}
saveRDS(gene_exp_summary1, here('rds/wt/03-expression_summary.rds'), compress=FALSE)
saveRDS(objs, here('rds/wt/03-objs_post_gene_filtering.rds'), compress=FALSE)
```
...
## aesthetics 
rmarkdown::render(here('processing_scripts/wt/03-gene_analysis.format.Rmd'),
                  output_file = '03-gene_analysis.html',
                  output_dir = here('reports/wt'))
```
