...
```{r}
objs <- readRDS(here('rds/02-annotated_objs.rds'))
tmp <- names(objs)[grepl('^PA', names(objs))]
tmp_func <- function(x) {
  num <- as.numeric(gsub('.+_(\\d)_.+', '\\1',x))
  y <- paste0('_V', num + 1, '_')
  gsub('_\\d_', y, x)
}
names(objs)[grepl('^PA', names(objs))] <- unlist(lapply(tmp, tmp_func))
```


```{r, eval=TRUE}
## Subset to 10x only for anna talk figures
objs <- objs[names(objs)[!grepl('5P', names(objs))]]
```
...
metadata$Kit <- factor(metadata$Kit, levels = c('NextGEM5P', 'GEMX5P', 'GEMX3P', 'NextGEM3P', 'Flex'))
metadata
```
...
filterLowGenes <- function(obj,
                           ...
                           assay = 'RNA',
                           calculate_only = TRUE){
  removed_genes <- (Matrix::rowSums(obj[[assay]]@layers$counts > 0) < min.cells)
  ...
  if (!calculate_only){
    ...
    # obj[[assay]] <- SeuratObject::CreateAssayObject(obj[[assay]]@layers$counts,
    #                                                 min.cells = min.cells)
    x <- rownames(obj@assays$RNA@features@.Data)[!removed_genes]
    obj <- subset(obj, features = x)
    # show(obj)
  }
  return(obj)
}
...
objs <- lapply(objs, function(x){
  x@misc$dim_receipts$raw_data <- dim(x)
...
plotdata2 <- group_by(plotdata, Kit) %>% 
  ...
  labs(x='Sample', y = 'Complexity ratio',
       title='Cell complexity: log10 genes / log10 UMI', 
...
plotdata <- lapply(objs, function(x){unname(x$log10GenesPerUMI)}) %>%
  ...
  labs(x='Kit', y='Complexity ratio',
       title='Cell complexity: log10 genes / log10 UMI') -> figures[['complexity_sum_boxplot']]
...
figures[['doublet_metric_dotplot']]$GEMX5P_F1
```
