...
```{r}
objs <- readRDS(here('rds/5p/wt/02-objs_post_cell_filtering.rds'))
...
shared_genes <- Reduce(intersect, detected_genes_kit_level)
# c("## Set of genes present in all 3' kits (not flex as it is probe based) in at least 10 cells in at least 2 samples",
shared_genes |>
  write.table(here('rds/5p/gene_whitelist.txt'), quote = FALSE, row.names = FALSE, col.names = FALSE)
length(shared_genes)
```
...
```{r}
saveRDS(gene_exp_summary1, here('rds/5p/wt/03-expression_summary.rds'), compress=FALSE)
saveRDS(objs, here('rds/5p/wt/03-objs_post_gene_filtering.rds'), compress=FALSE)
```
...
## aesthetics 
rmarkdown::render(here('processing_scripts/5p/wt/03-gene_analysis.format.Rmd'),
