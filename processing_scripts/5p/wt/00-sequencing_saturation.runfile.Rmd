...
```{r}
# zeroes <- data.frame(Sample=metadata_5p$Sample, nreads=0, median_genes=0, median_umi=0)
...
models_g <- lapply(unique(metadata_5p$Kit), function(kit) {
...
names(models_g) <- unique(metadata_5p$Kit)

models_u <- lapply(unique(metadata_5p$Kit), function(kit) {
  model_data <- sat_curves |>
    filter(Kit == kit) |>
...
names(models_u) <- unique(metadata_5p$Kit)
```
...
lapply(models_u, function(x) {
  ...
  write_plot_data(here('figure_data/5p/sequencing_saturation/model_fits_umi.txt'))
...
lapply(models_g, function(x) {
  ...
  write_plot_data(here('figure_data/5p/sequencing_saturation/model_fits_gene.txt'))
```

## Save models

```{r}
saveRDS(list(models_g = models_g, models_u = models_u), here('rds/5p/sat_models.rds'))
```
