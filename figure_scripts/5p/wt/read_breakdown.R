...
# Load data ----
fig_objs <- readRDS(here('rds/5p/wt/03-expression_summary.rds'))
gene_families <- list(
  'mitochondrial' = '^MT-',
  'ribosomal' = '^RP[SL]',
...
my_plot_save(image = figures[['Expression_barchart_shared']], 
             path = here('figures/5p/wt/read_utilization/shared_genes.svg'), 
             width = 8, height = 4)
plotdata |>
  ...
  write_plot_data(here('figure_data/5p/wt/read_utilization/shared_genes.txt'))
## Unique genes -----
...
my_plot_save(image = figures[['Expression_barchart_unique']], 
             path = here('figures/5p/wt/read_utilization/unique_genes.svg'), 
             width = 8, height = 4)
...
plotdata |>
  ...
  mutate(y = round(y * 100, 2)) |> 
  dcast(Kit + sample ~ group) |>
  write_plot_data(here('figure_data/5p/wt/read_utilization/unique_genes.txt'))
...
my_plot_save(image = figures[['Expression_barchart_all']], 
             path = here('figures/5p/wt/read_utilization/all_genes.svg'), 
             width = 8, height = 4)
