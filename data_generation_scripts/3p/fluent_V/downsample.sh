...
fastq_dir="/fh/fast/_IRC/FHIL/grp/BM02_FluentV5/fastqs/full"
outdir="/fh/fast/_IRC/FHIL/grp/BM02_FluentV5/fastqs/downsampled"

declare -A target_reads

target_reads["F1A"]=466400000
target_reads["F1B"]=440400000
target_reads["F5A"]=450775000
target_reads["F5B"]=475175000

for sample in "${!target_reads[@]}"; do
    # echo $sample ${target_reads[$sample]};
    for filetype in R1 R2; do
        seqtk sample -s100 ${fastq_dir}/*${sample}*${filetype}*.fastq.gz ${target_reads[$sample]} | pigz -p 36 > ${outdir}/B2-FBv5_${sample}_downsampled_${filetype}.fastq.gz
    done
done
